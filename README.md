...
- Go 1.26.7+ (to build from source — the `go` directive is pinned to the toolchain govulncheck verifies in CI, so stdlib CVEs surface as a failing build)
...
- **Owner tags** — uploads carry an owner tag (HMAC keyed from your master key) so the web UI can tell your uploads from those of connected apps or anyone else who can seal to your public key; `list` marks items without a valid tag as `[not your upload]`
